class Miku {
    constructor(game, x, y, spritesheet) {
        ...
        this.BB = new BoundingBox(this.x + 25, this.y, 42, 86, "player");
        this.lastBB = this.BB;
    ...
    drawLight(ctx2) {
        let x = this.BB.center.x;
        let y = this.BB.center.y - 20;
        let r = 80;
        ctx2.fillStyle = 'rgba(0,0,0,0.99)';
        ctx2.beginPath();
...
class mikuWalk {
    ...
    update(game, TICK) {
        ...
        if(Math.abs(stateManager.velocity.x) == MAX_WALK) {
            return new mikuRun(stateManager,this.direction);
        }
...
class mikuLand {
    constructor(stateManager) {
        ...
        this.landingDuration = this.stateManager.animations[this.name].totalTime;
        this.landingTime = 0;
    }
